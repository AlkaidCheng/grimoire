...
`scripts/gen_diff_html.py` (bundled with this skill) is a single Python file with no dependencies beyond `git` and Python 3.9+. Run it from the **repository root** (it shells out to `git` in the current directory):
...
- Use the **active coding agent's repository-local diff directory** (`.codex/diffs/`, `.claude/diffs/`, or whatever the running agent owns). Never put one agent's review artifacts in another agent's directory merely because that directory already exists.
...
- **Keep each `REVIEW.json` next to the page it produced**, named to match (`pr<NNN>_<slug>.json` produces `pr<NNN>_<slug>.html`). The index scans only `*.html`, so JSON inputs do not affect it.
