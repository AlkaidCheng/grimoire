...
The coding standard for C++: how it should be written and how it should be reviewed. The language-agnostic conduct — the change ethos (every change needs a reason; never change code just to change it) and the two working modes, **authoring** (apply the standard as you write; the default when implementing or modifying C++) and **reviewing** (audit against it and return an improved version per "How to Deliver the Review") — is defined in [`../_shared/review-conduct.md`](../_shared/review-conduct.md) and applies here in full.
...
- **Avoid accidental copies**: `for (auto x : vec)` copies each element — use `const auto&`; watch `std::map::operator[]` vs `find()`.
...
- **Rule of Zero/Five**: no direct resource management → declare no special members (compiler defaults are correct); managing a resource → implement all five or explicitly `= delete` the unsupported ones.
- **`const` correctness**: member functions, locals, parameters, getter returns — documentation plus a compile-time safety net.
...
- **`noexcept`**: destructors, move operations, and `swap` must be; mark others when you can guarantee no throw — enables optimizations and communicates intent.
...
Structural design — Single Responsibility, minimizing coupling, narrow and stable interfaces, and DRY (the same *fact* in two places, not lookalikes) — is owned by the `software-design` skill; apply it for those. The C++-specific mechanics:
...
- **Protect shared state**: `std::mutex` with `std::lock_guard` or `std::scoped_lock`; prefer `std::scoped_lock` for multiple mutexes — locks all at once, avoiding deadlock.
...
- **Const means thread-safe**: the standard library assumes `const` member functions are concurrently callable — protect mutated internals (caches, lazy init) with `mutable std::mutex`.
...
- **Secrets out of code**: no hardcoded API keys, passwords, or tokens — environment variables or restricted-permission config files.
