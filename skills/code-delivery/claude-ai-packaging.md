...
Companion reference to the code-delivery skill (`SKILL.md`), for the **Claude.ai** surface only — no direct repository access, so a code change ships as **inline fenced blocks** or as **zipped packages** surfaced through `present_files`. Everything not Claude.ai-specific stays in `SKILL.md`: pre-delivery checks, the PR-draft text artifacts ("Mode 3", identical on every surface), the human-facing-artifact rules, and the Claude Code path. Read this file only when packaging for Claude.ai.
...
Each commit gets its own zip. **The zip's internal paths must be relative to the repository root — no leading repo-name directory.** Files sit at their exact repo paths (`src/<pkg>/reader.py`, `tests/test_foo.py`) so the user unpacks each commit in sequence from the repo root:
...
A zip cannot express a deletion — extracting only adds or overwrites files. (The universal rule — list every removed path, mark it `(removed)` in the commit's file list — lives in `SKILL.md`.) If a commit removes files, call them out explicitly in chat with the exact removal commands, run from the repo root, alongside that commit's unzip step:
...
**Inline fix example.** The MSVC `__restrict__` → `__restrict` swap was small (one header + six call sites in one .cpp) but spanned two files, so it shipped as a focused fix zip, not inline — a zip applies more reliably than copying two code blocks into the right files. Rule of thumb: **if applying the change touches more than one file, zip it.**

**Iterative fix example (Mode 2).** The four-file Windows lock-offset fix: make the change; run build + tests + lint/type checks in one combined call; clean artifacts; build the focused fix zip (just the four files — no cumulative zip, none requested); `present_files`; then a body with the diagnosis table, root cause with doc citation, the fixes, verification numbers, and a quad-backtick commit message at the end.
