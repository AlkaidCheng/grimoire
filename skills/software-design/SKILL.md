...
- The opposite: an interface almost as complicated as its implementation — fifteen tiny forwarding methods, a "manager" that just passes calls along. Many tiny classes can be worse: the wiring between parts is its own complexity; more, smaller pieces is not automatically better.
- Every exposed option is something every future reader must learn — don't add one "in case someone needs it."
- Public-method test: *what it saves the caller vs. what the caller must learn* — about equal → not earning its place.
...
- **Don't split by execution order.** read → process → write, one class each, makes the reader and writer both know the format. Split by what each part hides, not by when the steps run.
...
- **Hard to name → the design is probably off.** The *and* test (SRP above): fix the design, not just the name. Honest exception: a top-level orchestration function — coordinating a sequence of steps *is* one job, as long as it reads top-to-bottom and hands the real work to named helpers.
...
- **DRY vs. keeping things separate** — a duplicated *fact* → remove it; lookalikes that will change for different reasons → leave them alone **[PP]** (see DRY above).
